...
class IEntityStorage {
 public:
  ...
  virtual std::optional<int> create(const std::string &resource,
                                    const std::string &data) = 0;

  // Retriecve an entity by ID, given resource (path) and ID, return data (JSON
  // string) Returns the data as a JSON string or nullopt if not found
  virtual std::optional<std::string> retrieve(const std::string &resource,
                                              int id) const = 0;

  // Update an entity by ID, given resource (path), ID, and new data (JSON
  // string) Returns true if successful, false otherwise
  virtual bool update(const std::string &resource, int id,
                      const std::string &data) = 0;
