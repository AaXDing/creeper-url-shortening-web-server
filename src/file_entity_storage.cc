...
FileEntityStorage::FileEntityStorage(const std::string &root_path)
    : root_(root_path) {
  fs::create_directories(root_);
}

// Generate next available integer ID by checking existing filenames
int FileEntityStorage::get_next_available_id(
...
// ID
std::optional<int> FileEntityStorage::create(const std::string &resource,
