...
protected:
    void SetUp() override {
        // Create handler args
        std::shared_ptr<NginxConfigStatement> stmt = std::make_shared<NginxConfigStatement>();
...
TEST_F(BlockingRequestHandlerTest, BlockingHandlerType) {
    EXPECT_EQ(handler_->get_type(), RequestHandler::HandlerType::BLOCKING_REQUEST_HANDLER);
}