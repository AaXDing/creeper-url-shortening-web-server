...
class Serv : public server {
    public:
        ...
        void call_start_accept() {
            start_accept();
        }
    
        // Expose private handle_accept(...)
        void call_handle_accept(ISession* sess,
...
public:
    explicit MockSession(asio::io_service& io)
      : sock_(io)
    {
...
TEST(ServerTest, HandleAccept_CallsStartOnSuccess) {
    ...
    Serv::SessionFactory factory = [mock]() {
        return std::unique_ptr<ISession>(mock.get());
    };

    Serv srv(ios, /*port=*/0, factory);
